...
From the main project directory (bn_tdd_pencil), run `rake run`

## To run tests:
Tests are run with RSpec.

From the main project directory (bn_tdd_pencil),
...
First run `cd spec/pencil_spec` to move into the pencil_spec directory (or simply `cd spec` for the display test) for individual method testing.
Add any of the following filenames after `rspec`:

* `write_spec.rb` - to test writing feature
* `point_deg_spec.rb` - to test point degradation and lead durability
* `sharpen_spec.rb` - to test sharpen feature
* `erase_spec.rb` - to test erase feature
* `erase_deg_spec.rb` - to test eraser durability
* `edit_spec.rb` - to test edit feature
* `display_spec.rb` - to test the terminal display

For example:
`rspec write_spec.rb --format documentation` would be run from inside of the spec/pencil_spec directory to test the requirements laid out in the writing section of the README.
